...
def handle_method(cfile, classname, winclassname, cppname, method, cpp, cpp_h,
                  existing_methods):
    ...
    if (cppname in manually_handled_methods
    ...
    for param in list(method.get_children()):
        if param.kind == clang.cindex.CursorKind.PARM_DECL:
            ...
            win_name = typename
            if (real_type.kind == clang.cindex.TypeKind.RECORD
                ...
                win_name = typename.replace(
                    real_type.spelling,
                    "win%s_%s" % (real_type.spelling, sdkver))
    ...
    should_gen_wrapper = method.result_type.spelling.startswith(
        "ISteam") or used_name.startswith("GetISteamGenericInterface")
        ...
        if param.type.kind == clang.cindex.TypeKind.POINTER:
            if not "const " in param.type.spelling:  # don't modify const arguments
                ...
                cpp.write("    lin_to_win_struct_%s_%s(&lin_%s, %s);\n" %
                          (real_type.spelling, sdkver, param.spelling,
                           param.spelling))
        else:
            cpp.write(
                "    lin_to_win_struct_%s_%s(&lin_%s, &%s);\n" %
                (param.type.spelling, sdkver, param.spelling, param.spelling))
    if method.result_type.kind != clang.cindex.TypeKind.VOID and len(
            need_convert) > 0:
...
""")
    ...
    for child in children:
        if (child.kind == clang.cindex.CursorKind.CXX_METHOD
            ...
            methods.append(
                handle_destructor(cfile, classnode.spelling, winclassname,
                                  child))
...
def handle_struct(sdkver, struct):
    ...
    if cb_num is None:
        ...
        if windows_struct64.get_size() != windows_struct.get_size():
            struct_name64 = "%s_%s" % (struct.displayname,
                                       windows_struct64.get_size())
    ...
    if not file_exists:
        ...
        cppfile.write('#include "steamworks_sdk_%s/isteamgameserver.h"\n' %
                      (sdkver))
        if os.path.isfile("steamworks_sdk_%s/isteamnetworkingsockets.h" %
                          sdkver):
            cppfile.write(
        ...
        if os.path.isfile("steamworks_sdk_%s/isteamgameserverstats.h" %
            ...
            cppfile.write(
                ...
                (sdkver))
        if os.path.isfile("steamworks_sdk_%s/isteamgamecoordinator.h" %
                          sdkver):
            cppfile.write(
    ...
    def handle_field(m, src, dst):
        if m.kind != clang.cindex.CursorKind.FIELD_DECL:
            return
        if m.type.kind == clang.cindex.TypeKind.CONSTANTARRAY:
            assert (m.type.element_type.kind != clang.cindex.TypeKind.RECORD
